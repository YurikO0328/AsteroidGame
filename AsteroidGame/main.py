...
asteroid50 = pygame.image.load("images/asteroid1.png")
asteroid100 = pygame.image.load("images/asteroid2.png")
asteroid150 = pygame.image.load("images/asteroid3.png")

shoot = pygame.mixer.Sound("soundfx/shoot.wav")
bangLargeSound = pygame.mixer.Sound("soundfx/bangLarge.wav")
bangSmallSound = pygame.mixer.Sound("soundfx/bangSmall.wav")
shoot.set_volume(.25)
...
class AlienBullet(object):
    def __init__(self,x ,y):
        ...
        self.w = 4
        self.h = 4
        self.dx, self.dy = player.x - self.xp, player.y - self.y
        self.dist = math.hypot(self.self.dx, self.dy)
        self.dx, self.dy = self.dx/self.dist, self.dy/self.dist
        self.xv = self.dx*5
        self.yv = self.dy*5

    def draw(self, win):
        pygame.draw.rect(win, (255,255,255), [self.x, self.w, self.h])
def redrawGameWindow():
    ...
    if rapidFire:
        ...
        win.blit(highScoreText,(sw-highScoreText.get_width()-22,25))
...
while run:
    ...
    if not gameover:
        ...
        for b in PlayerBullets:
            if(b.x>=a.x and b.x<=a.x+a.w) or b.x+b.w >= a.x and b.x+b.w<=a.x+a.w: 
                if(b.y>=a.y and b.y<=a.h)or b.y+b.h >=a.y and b.y+b.h <=a.y+a.h:
                    aliens.pop(i)
                if isSoundOn:
                    bangLargeSound.play()
                score += 50
        ...
        for i,b in enumerate(alienBullets):
            b.x += b.xv
            b.y += b.yv
            if(b.x>=player.x-player.w//2 and b.x <= player.x+player.y//2) or b.x+b.w>= player.x - player.w//2 and b.x+b.w<=player.x+player.w//2:
                if(b.x>=player.y-player.h//2 and b.y <= player.y+player.h//2) or b.y+b.h>= player.y - player.h//2 and b.y+b.h<=player.y+player.h//2:
                    lives -= 1
                    alienBullets.pop(i)
                    break
        ...
        for a in asteroids:
            ...
            if (a.x >+ player.x-player.w//2 and a.x <= player.x + player.w//2) or (a.x + a.w <= player.x + player.w//2 and a.x+a.w >= player.x-player.w//2):
                if (a.y>=player.y - player.h//2 and a.y <=player.y+player.h//2) or (a.y+a.h >= player.y-player.h//2 and a.y + a.h <= player.y+ player.y//2):
                    lives = 1
            ...
            for b in PlayerBullets:
                if(b.x >= a.x and b.x <= a.x + a.w) or b.x+b.w>=a.x and b.x+b.w<=a.x+a.w:
                    if(b.y >= a.y and b.x <= a.y + a.h) or b.y+b.h>=a.y and b.y+b.h<=a.y+a.h:
                        if a.rank == 3:
                            if isSoundOn:
                                bangLargeSound.play()
                            scoore += 10
        ...
        for s in stars:
            ...
            if s.x < -100 - s.w or s.x > sw + 100 or s.y > sh + 100 or s.y < -100 - s.h:
                ...
                break
            for b in PlayerBullets:
                if(b.x >= s.x and b.x <=+ s.w ) or b.x + b.w >= s.x and b.x+b.w <= s.x + s.w:
                    if(b.y >= s.y and b.y <=+ s.h ) or b.y + b.h >= s.y and b.y+b.y <= s.y + s.h:
                        rapidFire = True
                        rfStart = count
                        stars.pop(stars.imdex(s))
                        PlayerBullets.pop(PlayerBullets.index(b))
                        break
        if lives <= 0:
            gameover = True
    ...
    for event in pygame.event in pygame.event.get():
